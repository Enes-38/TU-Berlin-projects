...
element *insert_at_begin(element *first, element *new_elem) {
    /* HIER implementieren. */
	if (first == NULL) {   //wenn erster Elem 0 ist, dann
		first = new_elem;   //erster = neues element
	} else {
		new_elem->next = first;  //sonst neues Elemt wird zu erstes Elem
	}
	return new_elem;
}
 ...
 * Speicher frei.
 */
void free_list(list *alist) {    //Hier wird Speicher freigegeben
    /* HIER implementieren. */
